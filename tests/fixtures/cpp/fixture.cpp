...
/// The lone free function - exercises namespaced (non-overloaded) C++ calls.
int Square(int x) {
    return x * x;
}

/// The int overload - paired with the double overload below to exercise
/// overload resolution across the Extern "C++" boundary.
int Add(int a, int b) {
    return a + b;
}
